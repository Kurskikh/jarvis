...
settings-duck-level-desc = What percentage of their former volume everything else keeps. Lower is quieter: 20 is close to what Windows itself does during a call, 0 is silence. Higher barely changes anything - at 80 the difference is two decibels, which nobody can hear. If music is drowning jarvis out, start at 10-20. From 0 to 90.
settings-voice-volume = Voice volume
settings-voice-volume-desc = How loud jarvis himself is. 100 is as recorded. The recordings already sit close to the ceiling, so above 150 the loudest phrases can distort - there is not much room. If music is drowning him out, turn "How much is left" down first: there is far more room there. From 50 to 200.
